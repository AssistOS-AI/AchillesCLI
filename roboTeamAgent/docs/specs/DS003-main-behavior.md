...
Local instruction-skill selection is a live conversation policy. The default policy includes bundled copilot and conventional workspace catalogs; explicit empty selection stays empty. Each external turn or dequeued continuation captures current source files after queue wait. Active steering and explicit pinning keep their assigned revision. Settings and terminal continuations share the same policy reference. The CLI uses the configured policy but must not expose repository or skill administration commands. `/list robots` reads names and specializations from the current workspace robot store without ALA; terminal and WebChat must expose the same command catalog. DS006 defines capture, migration, scope and retention.
