...
Local instruction-skill selection is a live conversation policy. The default policy includes bundled copilot and conventional workspace catalogs; explicit empty selection stays empty. Each external turn or dequeued continuation captures current source files after queue wait. Active steering and explicit pinning keep their assigned revision. Settings, chat commands and terminal continuations share the same policy reference. DS006 defines capture, migration, scope and retention.
