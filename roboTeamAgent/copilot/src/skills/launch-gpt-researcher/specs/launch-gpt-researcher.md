...
`SKILL.md` routes to `node scripts/run.mjs --input '<task text or JSON>'`. The wrapper imports its local `scripts/ploinkyInvocation.mjs`, which loads `/Agent/client/AgentMcpClient.mjs` and calls Ploinky directly with the explicitly supplied CLI identity. `/run/ploinky-task/context.json` supplies the outer working directory and SDK configuration. One-way task receipts allow AchillesCLI to observe delegated tasks in the originating chat. No socket or MCP proxy is used. The wrapper awaits the action, prints its result and closes the invocation to flush provider results. All skill helpers remain in this folder.
