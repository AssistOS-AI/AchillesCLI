...
function createPathFilesystem(homeRoot) {
    return Object.freeze({
        ensureDirectory(directory, options) {
            return pathDirectory(homeRoot, directory, options);
        },
...
export function createContinuationStoreFixture(config) {
    return __testables.createContinuationStore(
        config,
        process.platform === 'linux' ? undefined : createPathFilesystem(config.homeRoot),
    );
}
