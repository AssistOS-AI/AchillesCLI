...
describe('copilot-router launcher integration', () => {
    ...
    it('routes online/current prompts to the Web Search launcher without touching Open Interpreter', async () => {
        const { agent, loopCalls, tempDir } = createRouterAgent();
        tempDirs.push(tempDir);
        const mcpCalls = [];
        const context = createWebchatContext({
            ...
            callAgentTool: async (...args) => {
                mcpCalls.push(args);
                throw new Error('Web search launcher should not call the Open Interpreter relay path');
            },
        });
        ...
        assert.equal(launcherResult.backend, 'web-search');
