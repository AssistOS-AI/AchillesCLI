...
The shared runtime-path helper must export `PYTHONDONTWRITEBYTECODE=1` before installation, UI startup, or MCP execution starts Python. This includes automatic `sitecustomize` imports and inherited child processes, so imports leave both the integration source and selected upstream checkout unchanged. The independent readiness command must use Python's `-B` option for the same bytecode policy. Launchers must not remove existing files or hide generated files from source-cleanliness checks.
