...
The adapter must pin the validated workspace and private root when it is constructed and revalidate that boundary before initializing or returning a cached AKU instance. Later environment hints must not retarget an existing adapter. The library's project root must remain the selected directory, including a project under `.data/achilles-cli/repos/`, while persistence stays under the pinned `aku/` root. A replaced `.data` ancestor must fail before storage access rather than redirecting a cached instance.
