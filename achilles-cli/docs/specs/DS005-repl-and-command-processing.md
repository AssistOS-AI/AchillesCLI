...
Task commands must use the AchillesCLI-owned journal under `.data/achilles-cli/tasks/`. `/tasks` and `/task view` are read-only. `/task stop` may cancel only the stored remote task. For terminal tasks, `/task continue` may use only the stored target, continuation tool, and opaque handle; it keeps the local task id while adding a new turn and appending the user's prompt before provider output. For ongoing tasks, the same command must use only the stored message tool and handle, require an explicit delivered or queued receipt, and keep the current remote task and turn. Providers without a declared message tool must reject live input.
