...
| `launch-robot` | Accepts a Desktop or Browser mode, robot name, and task; calls the matching RoboTeam start tool with the active workspace as `cwd`; polls `getTaskStatusForRobot`; and returns the live authenticated Selkies link when the graphical runtime is ready. |
...
RoboTeam calls must use `AgentMcpClient` through the Router and the target's internal workspace-agent tools; no delegated user identity is required because robots belong to the workspace. `launch-robot` must accept only Desktop and Browser modes, must not accept a replacement cwd, and may forward only the supported coding-agent, model, and skill-set hints. Its start call remains asynchronous, but the skill must poll the returned task id until the task reaches `running` or `completed` before presenting the session URL. RoboTeam may replace an idle same-mode GUI container when its existing cwd differs from AchillesCLI's active workspace. A failed or stopped task must produce a safe failure result rather than a link advertised as ready. RoboTeam retains task and container state; this initial integration does not add Take Control, stop, resume, or AchillesCLI task-journal ownership.
