...
A named launcher can activate a supported worker through Ploinky, submit work asynchronously, and return control to the conversation. Provider and RoboTeam launchers store the generic task journal and logs under `.data/achilles-cli/tasks/`, expose applicable lifecycle state through `/tasks` and `/task view`, and retain one local task identifier across supported continuation turns. WebChat must render the live log inside the task's collapsible conversation card. The expanded log must grow to a bounded height, follow appended output only while the reader remains at the bottom, and preserve the reader's position after an upward scroll. The collapsed card must retain the agent name, task description, lifecycle status, and elapsed time. The expanded card must link to the detailed view as `Open Task`, show `Stop` while work is active, and replace it with `Resume` only when a stopped task has a validated continuation handle. Stop and resume commands must pass through AchillesCLI's task manager instead of calling a downstream HTTP service from the browser.

The visible RoboTeam launcher submits workspace-scoped Desktop or Browser work through internal MCP, registers the asynchronous start with the background-task observer, waits only for the graphical runtime URL, and returns the authenticated Selkies session link. WebChat must open that link in its right panel while the task card remains in the conversation. Stopping must leave the RoboTeam workstation available for human input. Resuming must call the continuation tool and opaque handle recorded from the cancelled native task, attach the replacement remote task to the same local timeline, and let RoboTeam re-observe the human-modified screen before continuing. External agents keep their sessions and credentials private; [DS010](specsLoader.html?spec=DS010-ecosystem-integration.md) and [DS012](specsLoader.html?spec=DS012-launch-agent-skills.md) define the integration boundaries.
