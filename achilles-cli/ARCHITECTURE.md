...
AchillesCLI owns conversation sessions independently of WebChat. Each workspace stores validated session JSON under `.data/achilles-cli/sessions/`, while `.data/achilles-cli/settings.json` keeps `currentSessionId` beside the selected model and permission mode. Natural-language user/assistant turns enter this store as model context. Slash commands visibly submitted through WebChat, their visible responses, and any task references they create are retained in the same ordered session for presentation, but command records carry `context: false`; silent WebChat UI commands remain control traffic only.
