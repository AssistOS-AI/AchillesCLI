...
`/tasks [count|all]` lists the stored journal, `/task view <task-id>` reads a complete log, `/task stop <task-id>` cancels the stored remote task, and `/task continue <task-id> <prompt>` invokes the stored continuation tool and opaque handle. A continuation creates a new remote task but retains the local task id and increments its turn. AchillesCLI appends the submitted continuation prompt to the durable log before provider output and publishes that delta with the new offset. Terminal and WebChat autocomplete both display task names while inserting compatible ids for the selected action.
