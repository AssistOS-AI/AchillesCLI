...
The repository includes the AchillesCLI agent plus Ploinky manifests for the optional GPTResearcher, OpenCode, PI, and Codex workers used by launcher skills. The [agent documentation hub](index.html) explains every agent and links to its dedicated documentation. Detailed AchillesCLI runtime behavior is documented in [the technical documentation](docs/index.html), canonical terms are defined in [the wiki](docs/wiki.html), and requirements are indexed in [the specification matrix](docs/specsLoader.html?spec=matrix.md).
